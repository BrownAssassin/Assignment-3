...
int main() {
	...
	// Push another scope so most memory should be freed *before* we exit the app
	{
		...
		Texture2D::sptr shipHull = Texture2D::LoadFromFile("images/shipHull.png");
		...
		ShaderMaterial::sptr hullMat = ShaderMaterial::Create();
		hullMat->Shader = gBufferShader;
		hullMat->Set("s_Diffuse", shipHull);
		hullMat->Set("s_Specular", noSpec);
		hullMat->Set("u_Shininess", 8.0f);
		hullMat->Set("u_TextureMix", 0.0f);
		...
		GameObject obj2 = scene->CreateEntity("Spaceship");
		{
			VertexArrayObject::sptr vao = ObjLoader::LoadFromFile("models/PlayerShipX-WingFrame2.obj");
			obj2.emplace<RendererComponent>().SetMesh(vao).SetMaterial(hullMat);
			obj2.get<Transform>().SetLocalPosition(0.0f, 0.0f, 2.0f);
			obj2.get<Transform>().SetLocalRotation(90.0f, 0.0f, 0.0f);
			obj2.get<Transform>().SetLocalScale(glm::vec3(0.1, 0.1, 0.1));
			BehaviourBinding::BindDisabled<SimpleMoveBehaviour>(obj2);
		}

		GameObject obj3 = scene->CreateEntity("Mothership");
		{
			VertexArrayObject::sptr vao = ObjLoader::LoadFromFile("models/jellyfishBoss.obj");
			obj3.emplace<RendererComponent>().SetMesh(vao).SetMaterial(stoneMat).SetCastShadow(false);
		...
		EnvironmentGenerator::AddObjectToGeneration("models/simpleTree.obj", simpleFloraMat, 40,
			spawnFromHere, spawnToHere, allAvoidAreasFrom, allAvoidAreasTo);
		EnvironmentGenerator::AddObjectToGeneration("models/Debris.obj", boxMat, 15,
			spawnFromHere, spawnToHere, allAvoidAreasFrom, allAvoidAreasTo);
		EnvironmentGenerator::AddObjectToGeneration("models/simpleRock.obj", simpleFloraMat, 25,
